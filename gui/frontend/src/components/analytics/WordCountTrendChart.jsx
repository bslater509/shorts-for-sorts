...
const WordCountTrendChart = ({ perJobStats = [] }) => {
  ...
  const average =
    trendData.length > 0
      ? parseFloat((trendData.reduce((s, d) => s + d.wordCount, 0) / trendData.length).toFixed(1))
      : 0
  ...
  return (
    <div className="bg-card border border-border rounded-xl p-6">
      <h2 className="text-lg font-semibold mb-2">Content Length Trend</h2>
      <p className="text-xs text-muted-foreground mb-4">
        Word count per job across the batch sequence
      </p>
      {hasData ? (
        <div className="h-64">
        ...
        <div className="h-64 flex items-center justify-center text-sm text-muted-foreground">
          <div className="text-center space-y-2">
            <p>No content length data available.</p>
            <p className="text-xs opacity-60">Run jobs to see the word count trend.</p>
          </div>
        </div>
      )}
    </div>
